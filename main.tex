...
\begin{center}
    \huge{Arne Huckemann}\\
    \large{Intro to PDEs - Lecture Notes}\\
    \small{Prof. Martin Schmidt}
\end{center}
...
\]
If additionally this mapping is injective, then there is exactly one initial value dtermining the solution at $u(x(t),t)$. However, bijectivity alone does not suffice. We need further that this map and its inverse is continuously differentiable. However, the following condition in the theorem actually also suffices to show existence and uniqueness of the solution.
...
\exm{}{
    ...
    \end{cases}
    \]
    where $y(t)$ is the discontinuity that arises after $t=1$.
    \begin{figure}[H]
    \centering
...
\end{figure}
    ...
    Note that this solution exists if and only if the Rankine-Hugonoit condition is satisfied. The speed of the shock is the Rankine-Hugonoit condition 
    \[
...
Because the two solutions are equal at time $t=0$ due to the initial value, the monotonicity decreasing norm implies it is zero for all times. This implies uniqueness, if of the admissible solution if it exists.
\thm{Existence}{
...
\end{align*}
which is a $2n+1$ dimensional system and closed (as it only depends on $u$).\\
...
We will do here a generalization of the fundamental theorem of calculus to a higher dimension. First, we generalize partial integration. Second in what sense higher dimensional scalar convervation laws describe conserved quantity. This is will be just a implication of the divergence theorem.\\
\defn{}{
...
$$
and will see that information wrt. derivatives is retained in this linear form. The idea of distributions is to consider not just functions integrated against test functions, but all linear functionals defined on $C_0^\infty (\Omega)$. Meaning we generalize $F_f$ to arbitrary linear functionals, they do not have to be of the form of an integral against a function $f$. \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/Deflection.png}
    \caption{From NumPDE }
\end{figure}
...
The next result shows that smooth harmonic functions do not blow up in the interior of a domain, by showing that all their partial derivatives are bounded. On an open ball $B_r (x)$ the value of any partial derivative of a smooth harmonic function in the center of that ball is bounded by above by a finite constant depnding on the radius and the supremum on the closure of the ball. Liouvilles Theorem will then use this result and show that bounded harmonic functions are constant. This can be seen in the form of the upper bound of the following corollary, as this bound goes to zero as $r \to \infty$.
    ...
    For the following theorem we need that $u$ is also smooth. However, soon we will show in Weyl's Lemma that harmonic distributions are smooth functions and thus we can assume smoothness without loss of generality.
    \thm{Liouvilles}{
...
\]
This implies $I(u-v)=0$
}

\section{A PDE with no solutions}
\lem{}{
    Let $\Omega = \mathbb{R}^2 \setminus \{0\}$. The only twice differentiable function $u: \Omega \to \mathbb{R}$ that satisfies 
    \[
    - |x|^2 \Delta u \geq u^2,
    \]
    is the trivial solution $u \equiv 0$.
}
We can now construct a PDE with no solution. Consider the PDE $-|x|^2 \Delta u =u^2 +1$ with any boundary conditions. Then due to 
\[
-|x|^2 \Delta u =u^2 +1 \geq u^2,
\]
it follows with the lemma above that the only possible solution is $u \equiv 0$. But this is not a solution of the PDE, since $- |x|^2 \Delta 0 =0 \neq 1 =0^2 +1$. Thus this PDE has no solution.
...
We now focus on $\frac{\partial}{\partial t} u - \Delta u=0$ the heat eqaution and is inhomogenous form $\dot{u} - \Delta u=f$, where $u$ is defined on the open domain $\Omega \subset \mathbb{R}^n \times \mathbb{R}$ and $f$ on $\Omega$. We will extend statements about harmonic function to solution of the heat equation. This equation models the diffusion of some quantity. The heat equation follows from the scalar conservation law: Consider 
\[
\frac{\partial}{\partial t} u(x,t) + \nabla \cdot F(u(x,t)) = 0, \quad (x,t) \in \Omega, \quad F := -\nabla.
\]
Then the heat equation follows:
\[
\frac{\partial}{\partial t} u(x,t) + \nabla \cdot (- \nabla u(x,t)) = \frac{\partial}{\partial t} u(x,t) - \Delta u(x,t) = 0.
\]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/OmegaT.png}
    \caption{From \cite{evans2010partial} }
\end{figure}
...
We see that if $s \to t$ then the right hand side goes to $0$. The heat ball has to be at that time a singular point. Further, if we let $s \to t- \frac{r}{4\pi}$ then the terms inside the logarithm go to $1$ and thus the left hand side goes to $0$. Thus the heat ball is some kind of paraboloid in space-time.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/Heat Ball.png}
    \caption{From \cite{evans2010partial} }
\end{figure}
...
\pf{
    ...
    \[
    E(0,0,r) = \{ (y,s) \in \mathbb{R}^{n+1} \mid \psi (y,s) \geq 0, s \leq t \}
    \]
...
\end{align*}
The above is for $x \in \mathbb{C}$ equivlanet to 
\[
 \pi^{n/2} e^{x^2} = \int_{\mathbb{R}^n} e^{-(k-x)^2+x^2} d^n k  \quad \iff \quad \pi^{n/2}  = \int_{\mathbb{R}^n} e^{-(k-x)^2} d^n k
\]
Using this we get
\begin{align*}
...
\[
\mathcal{F}(\Delta h) = 4 \pi^2 k^2 \hat{h}(k) \quad \iff \quad \Delta h = \mathcal{F}^{-1} (4 \pi^2 k^2 \mathcal{F}(h)).
\]
...
\[
\int_0^1 \sqrt{2} \sin (k \pi x) \sqrt{2} \sin (k^\prime \pi x) dx = \int_0^1 \cos( (k-k^\prime) \pi x ) - \cos ((k+k^\prime ) \pi x) dx = \delta_{k,k^\prime}
\]
$(\sqrt{2} \sin (k \pi x))_{k \in \mathbb{N}}$ builds an orthonormal system of $C(0,1)$. Note that there exists a bijection between $(0,1)$ and $\mathbb{R} / 2\mathbb{Z}$. Let $f$ be continuous on $[0,1]$ with roots at $\partial (0,1)$. Define the the odd reflection 
\[
\hat{f} (x) := \begin{cases}
    f(x) &, x \in (0,1) \\
    -f(-x) &, x \in (-1,0)
...
\end{cases}
\]
is continuous on $\mathbb{R}$ and has roots on $\mathbb{Z}$ and period $2$. Because $(e^{i \pi k x})_{k \in \mathbb{Z}}$ builds an ONB of $L^2(\mathbb{R}/ 2 \mathbb{Z})$, we can write
\begin{align*}
...
\end{align*}
Interpretation: Theta functions are a tool from function theory. We only reference their connection to this field of mathematics without going into detail.\\
...
\rmkb{
    In general there are two approaches in solving the wave equation. The method of spherical means defines a PDE on the averages of the solution, which transforms the problem into a one dimensional problem and then lets the radius go to zero. Then using Kirchhoffs formula, which will be introduced, we get a formulation for the solution. This is applicable in odd dimensions.\\
    The method of descent defines the $n$ dimensional problem onto $n+1$ dimensions, where howevery, the initial data does only depend on $n$ dimensions. By transforming the solution in $n+1$ onto $n$ dimensions, the solution can be recovered. In even dimensions, this is the only approach possible. Though, in odd dimensions this can also be done.
}
...
\pf{
    Only constant needs to be calculated.
}


Note that the method of descent also works for odd dimensions. We can define the problem in $n+1$ dimensions, i.e. for $n$ odd, by letting the initial values only depend on $n$ variables. Then we can indeed recover the solution on $\bar{x} \in \mathbb{R}^n$ by evaluating the solution in $x \in \mathbb{R}^{n+1}$ at $x_{n+1}=0$, similiar to before.
