...
\rmkb{
    First read the section to the Fourier Transform. Here the connection of the heat equation to an ODE and the fundamental solution is discussed to motivate Duramels principle. The inhomogeneous heat equation $\dot{u}= \Delta u +f$ can be viewed as an ODE on an infinite functional space on which $u$ is defined, i.e. we have the operator $H:= \Delta$. Then the solution to this linear ODE is given by 
    \[
...
\begin{align*}
    \pi^{n/2} e^{x^2}&= \pi^{n/2} \sum_{l=0}^\infty \frac{|x|^{2l}}{l!} = \int_{\mathbb{R}^n} e^{-(k-x)^2} d^n k \sum_{l=0}^\infty \frac{|x|^{2l}}{l!} \\
    &= \int_{\mathbb{R}^n} e^{-(k-x)^2+x^2} d^n k = \int_{\mathbb{R}^n} e^{-k^2 + 2kx} d^n k = \int_{\mathbb{R}^n} e^{-k^2 + 2kx} d^n k,
\end{align*}
\lem{}{
    If two analytical function agree on the real axis, then they agree on the entire complex plane. 
}
\thm{}{
...
???THis is insted used???\\
We now need to show that the last integral is an analytical function, because only then we have can imply that this translation is possible on the entire complex plane. Note that the first term is analytical, but that does necessarly imply that the last term is also analtical. It holds 
\[
\int_{\mathbb{R}^n} e^{-k^2 + 2kx} d^n k= \sum_{l=0}^\infty \int_{\mathbb{R}^n} e^{-k^2} \frac{(2kx)^l}{l!} d^n k =  \sum_{l=0}^\infty \int_{\mathbb{R}^n} e^{-k^2} \frac{(2kx)^{2l}}{(2l)!} d^n k .
\]
Using this we get
\begin{align*}
...
This then yields 
\begin{align*}
    h(x)&=\lim_{t \searrow 0} u(x,t) = \lim_{t \searrow 0} \int_{\mathbb{R}^n} \Phi (x-y,t) h(y) d^n y\\
    ...
    &=  \lim_{t \searrow 0}   \int_{\mathbb{R}^n} e^{ - 4 \pi^2 k^2 t } e^{2 \pi k  i x} \hat{h} (k)  d^n k 
\end{align*}
...
Because $W_0^{2,2}(\Omega)$ is defined to be complete, these limits exist.\\
...
The Laplace operator $- \Delta : W_0^{2,2} (\Omega) \to ???$ is closed due to the construction of $W_0^{2,2} (\Omega) := \overline{ C_0^\infty(\Omega)}^{\| \cdot \|_{W_0^{2,2} }}$. This is because by construction $h_n \to h$, $\Delta h \to v$ and $\Delta h =v$. The Laplacian is further self-adjoint, because it is symmetric due to Greens second identiy:
\[
\langle - \Delta u,v \rangle_{L^2} = \int_\Omega \nabla u \overline{\nabla u} d^n x = \langle u, - \Delta v \rangle_{L^2} \text{ and } \langle -\Delta u,u \rangle_{L^2 } = \| \nabla u \|^2 \geq 0.
\]
Thus, (???) it follows that $-\Delta$ has a spectral decomposition. Then, (???) it follows that 
\[
u(x,t) = \left( e^{-tH} h \right)(x) \text{ and } \| e^{-tH} h \|_{L^2(\Omega)} \leq const. \| h \|_{L^2(\Omega)} < \infty
\]
and solves 
\[
\dot{u} (x,t) = - \left( H e^{-tH} \right)(x) \stackrel{Def.}{=} \Delta u(x,t)
\]
and also satisfies the Dirichlet boundary conditions 
\[
u(x,0)= \left( e^0 h \right)(x)=h(x) \text{ and } \forall x \in \partial \Omega u(x,t) =0.
\]
The last equality holds because $h \in C_0^\infty (\Omega)$.
...
The Eigenfunction of $- \frac{d^2}{dx^2}$ on $(0,1)$ that satisfies the Dirichlet fboundary conditions, i.e. having their roots at $\partial (0,1) = \{0,1\}$, are given by $\sqrt{2} \sin (k \pi x), k \in \mathbb{N}$: 
\[
