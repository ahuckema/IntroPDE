...
\end{equation}
Let $A_i$ be a cover of $\partial B_R (0)$ and $O_i$ a cover of $S^{n-1}$, then due to $\psi \circ F= T \circ \phi $
\begin{align*}
...
We define the solution of the Laplace equation as
\[
\Phi (x) :=\begin{cases}
...
\begin{align*}
    \int_{S^{n-1}} 1 d \sigma (x) &= \int_{S^{n-1}} |x|^2 d \sigma (x) = \int_{S^{n-1}} F \nu d\sigma = \int_{B_1 (0)} div F dx \\
    &= \int_{B_1 (0)} \sum_{i=1}^{n} \frac{\partial}{\partial x_i} x_i dx = n \lambda^{n} (B_1(0)) =: n \omega_n
\end{align*}
it finally follows
\begin{align*}
\int_{B_\epsilon (0)} |\Phi (y) | d^n y& = \int_0^\epsilon \int_{S^{n-1}} |\Phi (r x )| d \sigma(x) r^{n-1} dr = \int_0^\epsilon r^{n-1} | \Phi (r e) | n \omega_n dr \\
&= \int_0^\epsilon r^{n-1} | \Phi (r e) | n \omega_n (x) dr \\
&= \begin{cases}
...
\end{cases}
\end{align*}
Note that in the case $n=2$ we have $\omega_n = \pi$. Both terms are finite! And as the upper bounds depend on $\epsilon$ only we have that $I_\epsilon \to 0$ as $\epsilon \to 0$.
