...
\pf{
    First, we shall find a $u$ as it was proposed in the lemma. For every $x \in \Omega$ choose a ball $B_r (x) \subset \Omega$ and a test function $\psi \in C_0^\infty ((0,r))$ with $\int_0^r \psi (s)ds =1$. Then we define 
    \[
    u(x) := u(g_x), \quad g_x (y) := \frac{\psi (|y-x|)}{n |y-x|^{n-1} \omega_n}.
    \]
Now we need to show that $u$ is well defined, i.e. independent of the choice of $r$ and $\psi$. For that define 
\[
g_{x,1} (y) := \frac{\psi_1 (|y-x|)}{n |y-x|^{n-1} \omega_n}, \quad g_{x,2} (y) := \frac{\psi_2 (|y-x|)}{n |y-x|^{n-1} \omega_n}, 
\]
for $\psi_1 \in C_0^\infty ((0,r_1))$ and $\psi_2 \in C_0^\infty ((0,r_2))$. We want to show that $U(g_{x,1})= U(g_{x,2})$ which is equivalent by linearity to $U(g_{x,1}-g_{x,2})=0$. For ease of notation define 
\[
f(y) := \frac{\psi_1 (|y-x|)- \psi_2 (|x-y|) }{n |y-x|^{n-1} \omega_n}.
\]
Thus we need to show that $U(f)=0$, where $\psi := \psi_1 -\psi_2$. We do this by showing that the conditions from the mean value theorem are satisfied. It holds that 
\[
\int_0^{\max {r_1,r_2}} \psi ds = \int_0^{r_1} \psi_1 ds - \int_0^{r_2} \psi_2 ds = 1-1=0.
\]
Thus $u$ is independent from $\psi$ and $r$. Additionally it holds that 
\[
