...
\lem{}{
    Let $(X,\| \cdot \|_X)$ be a normed space, $(Y,\|\cdot \|_Y)$ a Banach space, $D$ a dense subspace of $X$ and $T \in L(D,Y)$ a linear operator. Then there exists only one unique continuous continuation $\hat{T} \in L(X,Y)$ such taht $\hat{T}_{\big|D}=T$ and $\| \hat{T} \| = \| T \|$.
    }
\pf{
