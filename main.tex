...
\dot{u} + b \cdot \nabla u =f ,
\]
linear inhomogeneous transport equation.
\defn{Initial Value Problem (Cauchy Problem)}{
For $f: \mathbb{R}^n \times \mathbb{R} \to \mathbb{R}$, $b \in \mathbb{R}^n$ we seek to find a solution $u: \mathbb{R}^n \times \mathbb{R} \to \mathbb{R}$ of the linear homogenuous transport equation 
$$
\dot{u}+b \cdot \nabla u=0, \quad u(x,0)=g(x)
$$
where $g: \mathbb{R}^n \to \mathbb{R}$ is some function.
...
\[
\frac{d}{dt} \int_a^b u(x,t) = \int_a^b \dot{u} (x,t) = - \int_a^b \frac{\partial}{\partial x} g(u(x,t)) dx = f(u(a,t)) - f(u(b,t)).
\]
...
\[
z^\prime (s) = \frac{\partial}{\partial x} u(x(s) , t(s) ) x^\prime (s) + \frac{\partial}{\partial t} u(x(s),t(s)).
\] 
Choose $x^\prime (s) := f^\prime (u(x(s),t(s)))$ and $t^\prime (s)=1$, then we have 
\[
z^\prime (s) = \frac{\partial}{\partial x} u(x(s) , s ) f^\prime (s) (u(x(s),s) ) + \dot{u}(x(s),s) =0 ,
\]
...
\pf{
    ...
    \begin{itemize}
        ...
        \item Let $t \geq $ such that $1-t \alpha \leq 0$:\\
...
\[
 \int_a^b  \frac{\partial}{t} u(x,t) dx = - \int_a^b \frac{\partial}{\partial x} f(u(x,t)) dx = f(u(a,t)) - f(u(b,t)).
\]
Thus, if we assume that $u$ is not differentiable we can formulate this initial value problem as 
\[
 \frac{\partial}{t} \int_a^b   u(x,t) dx = f(u(a,t)) - f(u(b,t)).
\]
...
\thm{Uniqueness}{
    Let $f \in C^1 (\mathbb{R},\mathbb{R})$ be conves and $u$ and $v$ two admissible solution of 
    \[
...
\begin{itemize}
    ...
    \]
    and show that $F_u=:\tilde{U}$ satisfies the weak mean value property, $u$ is harmonic and $U=\tilde{U}$. This only works if $u$ is constructed independently from $\phi$ and $r$.
\end{itemize}
...
\begin{itemize}
 ...
 Since $\psi \in C_0^\infty ((0,r))$ the function
 \[
...
\]
This is an inhomogeneous transport equation with constant coefficients. Let $X(s) := x+t-s$ and $U(s) := u(X(s),s)$, then
\[
...
\pf{
    ...
    \begin{align*}
        ...
        &= \frac{1}{n \omega_n} \frac{r^n}{r^n} \int_{ B_1 (0)}  \nabla \cdot \nabla u (x+ry,t) d^n y = \frac{1}{n } \fint_{ B_1 (0)}  \nabla \cdot \nabla u (x+ry,t) d^n y \\
...
\]
Using the continuity of $r \mapsto \tilde{U} (x,r,t)$ we get
\[
u(x,t) = \lim_{r \searrow 0} U(x,r,t) = \lim_{t \searrow 0} \frac{\tilde{U}(x,r,t)}{r},
\]
which implies for all $x \in \mathbb{R}^3, t >0$:
\begin{align*}
