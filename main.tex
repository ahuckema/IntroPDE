...
We want the characteristics curves $x(s)$ to move away from $H$ to $\Omega$, i.e. their slope needs to be orthogonal to $H$, which is formally 
\[
\dot{x}(s)^T e_n \neq 0.
\]
We can transform the PDE into an ODE wrt. the characteristics: We do this by differentiating the ODE. For $p(x) := \nabla u(x)$ we have
\begin{align*}
    0 &= \frac{\partial}{\partial x_i} F(\nabla u(x),u(x),x) = \sum_{j=1}^n \frac{\partial}{\partial p_j} F(p(x),u(x),x) \frac{\partial}{\partial x_i } p(x) + \frac{\partial}{\partial u(x)} F(p(x),u(x),x) \frac{\partial}{\partial x_i} u(x) + \frac{\partial}{\partial x_i} F(p(x),u(x),x)\\
    &= \sum_{i=1}^n \frac{\partial}{\partial p_i} F(p(x),u(x),x) \frac{\partial}{\partial x_i } p(x) + \frac{\partial}{\partial u(x)} F(p(x),u(x),x) \frac{\partial}{\partial x_i} u(x) + \frac{\partial}{\partial x_i} F(p(x),u(x),x)\\
    &\iff \sum_{i=1}^n \frac{\partial}{\partial p_i} F(p(x),u(x),x) \frac{\partial}{\partial x_i } p(x) = - \frac{\partial}{\partial u(x)} F(p(x),u(x),x) \frac{\partial}{\partial x_i} u(x) - \frac{\partial}{\partial x_i} F(p(x),u(x),x)
\end{align*}
If we now consider the time differential of $p(x) = \nabla u(x)$ along the characteristic
\[
\frac{d}{ds} \nabla u(x(s)) = \sum_{j=1}^n \frac{\partial}{\partial x_j} \nabla u(x(s)) \dot{x}_j (s)
\]
we have 
\[
 \sum_{j=1}^n \frac{\partial}{\partial x_j} \nabla u(x(s)) \dot{x}_j (s) = - \frac{\partial}{\partial u(x)} F(p(x),u(x),x) \frac{\partial}{\partial x_i} u(x) - \frac{\partial}{\partial x_i} F(p(x),u(x),x) \quad \iff \quad \dot{x}_j (s)= \frac{\partial}{\partial p_i} F(p(x),u(x),x) .
\]
