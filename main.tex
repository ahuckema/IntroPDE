...
Define $\{ z \in \Omega \mid u(z) = M \}$. We have shown in the above that $A \neq \emptyset $ because $B_r (x) \subset A$. We will show that $A$ is closed an open an non-empty. This implies due to (*) that $A = \Omega$. \\
...
\]
Since $A$ is closed it follows that $A^C$ is closed. Because $\Omega = A \cup A^C$ and $\Omega$ is connected one of the sets has to be empty. As $A \neq \emptyset$ it follows $A^C = \emptyset$.
}
...
\pf{
    ...
    Due to the fact that $G_{B_r (0)} (x,0)$ vanishes on $\partial B_r (0)$ it follows that for $x_0 \in \partial B_r (0)$ $\tilde{u}(x_0)= u(x_0)$, which yields $u_\epsilon (x_0) =0$. Further, as stated before, $G_{B_r (0)}(x,0)$ is singular in $x=0$. Due to the fact that $\tilde{u}$ and $u$ are bounded it follows for every $\epsilon >0$ that $u_\epsilon (x) \to \infty, x \to 0$. Note that $u_\epsilon$ is a harmonic function on $B_r (0) \setminus \{0\}$ (linear combination of harmonic functions).\\
