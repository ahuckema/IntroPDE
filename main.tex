...
Unlike the Laplace equatin the solution to the heat equation satisfies a mean value property on a space time region (not a ball) called heat balls $E(x,t,r)$, which is defined through $\Phi$.\\
...
If $\Omega$ is open and boundad and $u \in C^2$ is the solution to the heat equation extending constinuousky to $\bar{\Omega_T}$, it follows that the maximum is on $\partial \Omega_T$. Using these we can show the existence and uniqueness of the Cauchy problem on an open and bounded domain.\\
...
\]
Thus, we have for $b(x+t) := u(x+t,0)$ that 
\[
u(x,t) = \int_0^t a(x+t-2s) ds + b (x+t) \stackrel{y= x+t-2s, \quad dy=(-2) ds}{=} \int_{x+t-2\cdot 0}^{x+t-2t} (- \frac{1}{2}) a(y) dy + b(x+t) =  \frac{1}{2} \int_{x-t}^{x+t} a(y) dy + b(x+t).
\]
...
we get 
\begin{align*}
    u(x,t) &= \frac{1}{2} \int_{x-t}^{x+t} g(y) - g^\prime (y) dy + g(x+t)\\
...
Later in this chapter the following will be of importance. Consider the initial value problem 
\begin{align*}
...
\end{align*}
Due to $\lim_{t \to 0} U(x,r,t) =u(x,t)$ it follows 
\[
\tilde{U} (x,0,t) = \lim_{t \to 0} r U(x,r,t,) =0 \cdot u(x,t)=0.
\]
...
\begin{align*}
    u(x,t) &= \lim_{r \searrow 0} \frac{1}{2} \left( \frac{\tilde{G}(x,t+r)- \tilde{G}(x,t-r)}{r} \right) + \lim_{r \searrow 0} \frac{1}{2r} \int_{t-r}^{t+r} \tilde{H} (x,s) ds\\
